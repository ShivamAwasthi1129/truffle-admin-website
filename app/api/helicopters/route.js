...
export async function POST(request) {
  try {
    ...
    if (!itemData.name || !itemData.model) {
    ...
    const lastIdNum = lastItem ? parseInt(lastItem._id.replace('HC', '')) : 0;
    const id = `HC${String(lastIdNum + 1).padStart(3, '0')}`;
    ...
    const newItem = {
      ...
      base_location: itemData.base_location || {},
