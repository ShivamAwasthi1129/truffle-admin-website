...
function parseId(id) {
  try {
    ...
    // Otherwise, return the string as is (for custom IDs like YT001)
    return id;
  } catch (error) {
...
export async function PUT(request, { params }) {
  try {
    ...
    const allowedFields = [
      ...
      'max_speed', 'range', 'last_maintenance', 'insurance_expiry', 'registration_no', 'cabin_height', 'engine_type'
    ]
    ...
    if (filteredUpdateData.length_m) filteredUpdateData.length_m = parseFloat(filteredUpdateData.length_m);
    if (filteredUpdateData.cabins) filteredUpdateData.cabins = parseInt(filteredUpdateData.cabins);
    ...
    if (filteredUpdateData.base_marina) {
      if (filteredUpdateData.base_marina.lat) filteredUpdateData.base_marina.lat = parseFloat(filteredUpdateData.base_marina.lat);
      if (filteredUpdateData.base_marina.lng) filteredUpdateData.base_marina.lng = parseFloat(filteredUpdateData.base_marina.lng);
    }
    ...
    if (result.modifiedCount === 0) {
      return NextResponse.json({ 
        message: 'No changes made to yacht',
        item: existingItem 
      });
    }
