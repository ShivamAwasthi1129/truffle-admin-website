...
function parseId(id) {
  try {
    ...
    // Otherwise, return the string as is (for custom IDs like CF001)
    return id;
  } catch (error) {
...
export async function GET(request, { params }) {
  try {
    ...
    if (!item) {
      return NextResponse.json({ error: 'Charter flight not found' }, { status: 404 });
    }

    return NextResponse.json({ item });
  } catch (error) {
    console.error('Error fetching charter flight:', error);
...
export async function PUT(request, { params }) {
  try {
    ...
    if (result.modifiedCount === 0) {
      return NextResponse.json({ 
        message: 'No changes made to charter flight',
        item: existingItem 
      });
    }
    ...
    return NextResponse.json({ 
      message: 'Charter flight updated successfully', 
      item: updatedItem 
    });
  } catch (error) {
...
export async function DELETE(request, { params }) {
  try {
    ...
    return NextResponse.json({ message: 'Charter flight deleted successfully' });
  } catch (error) {
