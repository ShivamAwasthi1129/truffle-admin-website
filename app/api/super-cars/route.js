...
export async function POST(request) {
  try {
    ...
    const lastIdNum = lastItem ? parseInt(lastItem._id.replace('SC', '')) : 0;
    const id = `SC${String(lastIdNum + 1).padStart(3, '0')}`;
    ...
    const newItem = {
      ...
      horsepower: itemData.horsepower || 0,
    ...
    if (result.insertedId) {
      return NextResponse.json({ 
        message: 'Super car created successfully', 
        item: newItem 
      }, { status: 201 });
    } else {
      return NextResponse.json({ error: 'Failed to create super car' }, { status: 500 });
    }
  } catch (error) {
